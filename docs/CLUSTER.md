...
- 客户端“请求并发”只决定同时挂起多少个 HTTP 请求；真正的生成并行数是该模型在所有节点上的 worker 副本数之和。例如 Mac 的 CosyVoice 2 个副本加 Windows 1 个副本，实际就是 3 路，其余请求留在协调端队列等待。
- 调度按模型槽位认领任务，不会再用空闲的 system/F5 槽位预占 CosyVoice 任务。Mac 内置任务和远程任务都会续租，慢任务不会因超过 `lease_ttl` 被重复派发。
