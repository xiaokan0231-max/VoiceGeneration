...
export default function TaskCenter() {
  ...
  const { tasks, activeCount, cancelGeneration, retryGeneration } = useGeneration()
  const { notify } = useToast()
  const queued = tasks.filter(task => task.status === 'queued').length
  const working = tasks.filter(task => task.status === 'leased' || task.status === 'running').length
  ...
  return <div className="task-center">
    <button className={`task-trigger ${activeCount ? 'active' : ''}`} aria-expanded={open} aria-controls="task-drawer" onClick={() => setOpen(value => !value)}>
      {activeCount ? <LoaderCircle className="spin" /> : <Clock3 />}
      <span>任务</span><strong>{activeCount}</strong>
    </button>
    {open && <>
      ...
      <aside className="task-drawer" id="task-drawer" aria-label="生成任务中心">
        ...
        <div className="task-list">
          {tasks.length === 0 && <div className="task-empty"><CheckCircle2 /><p>当前没有生成任务</p><small>从工作台提交后，可在任意页面查看进度。</small></div>}
          {tasks.map(task => <article className={`task-item ${task.status}`} key={task.id}>
