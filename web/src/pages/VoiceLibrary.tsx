...
import { FileAudio, Mic, Pause, Play, Plus, Save, Square, Trash2, Upload, X } from 'lucide-react'
import { api } from '../api'
...
export default function VoiceLibrary() {
  ...
  const save = async () => {
    ...
    try { await api.saveVoice(form, editing.existing ? editing.id : undefined); setEditing(null); await load() }
    catch (e) { setError(e instanceof Error ? e.message : '保存失败') }
    finally { setSaving(false) }
  }
  const remove = async (voice: VoiceDetail) => {
    if (!window.confirm(`删除音色“${voice.name}”？已生成的历史音频不会删除。`)) return
    try { await api.deleteVoice(voice.id); await load() } catch (e) { setError(e instanceof Error ? e.message : '删除失败') }
  }
  ...
  return <div className="content-page">
    ...
    <div className="voice-list">
      {voices.map(voice => <article className="voice-row" key={voice.id}>
        ...
        <div className="row-actions"><button onClick={() => edit(voice)}>编辑</button><button className="danger-icon" onClick={() => remove(voice)} aria-label="删除音色"><Trash2 /></button></div>
      </article>)}
    ...
    {editing && <div className="dialog-backdrop"><section className="dialog-panel" role="dialog" aria-modal="true">
      <div className="dialog-head"><div><span className="eyebrow">VOICE PROFILE</span><h2>{editing.existing ? '编辑音色' : '新建音色'}</h2></div><button className="icon-button" onClick={() => setEditing(null)}><X /></button></div>
      <div className="field-row"><div className="field"><label>音色名称</label><input value={editing.name} onChange={e => set('name', e.target.value)} placeholder="例如：沉稳男声" /></div><div className="field"><label>语言</label><select value={editing.language} onChange={e => set('language', e.target.value)}><option value="zh">中文</option><option value="ja">日语</option><option value="en">英语</option></select></div></div>
      {!editing.existing && <div className="field"><label>音色 ID（可选）</label><input value={editing.id} onChange={e => set('id', e.target.value)} placeholder="留空则自动生成" /></div>}
      <div className="field"><label>参考音频逐字稿</label><textarea value={editing.refText} onChange={e => set('refText', e.target.value)} placeholder="请准确填写录音中说的文字" /></div>
      <div className="upload-zone"><input type="file" accept="audio/wav,audio/mpeg,audio/mp4,audio/webm" onChange={e => set('file', e.target.files?.[0])} /><Upload /><strong>{editing.file ? editing.file.name : editing.existing ? '保留当前参考音频，或点击替换' : '点击选择 WAV / MP3 / WebM'}</strong><span>最大 20MB，时长 3–30 秒</span></div>
      ...
      <div className="dialog-actions"><button className="quiet-button" onClick={() => setEditing(null)}>取消</button><button className="primary-small" disabled={saving} onClick={save}><Save />{saving ? '保存中…' : '保存音色'}</button></div>
    </section></div>}
  </div>
}
